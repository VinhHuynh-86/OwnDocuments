...
namespace VH_SYNC
{
    ...
    public class SyncTransform : NetworkBehaviour
    {
        ...
        private float m_LastServerSendTime;
        ...
        private float m_Latency = 0.02f; //1f / FixedSendsPerSecond; // = 0.02f; // should be RTT. Round trip time (RTT) latency: the time for input to go from client to game server and the result goes back to client.
        // can find by ping to server and receive the result
        private float m_ClientHistoryDuration;
        ...
        void FixedUpdate()
        {
            ...
            if (NetworkManager.Singleton.NetworkTime - m_LastSendTime >= (1f / FixedSendsPerSecond))
            {
                ...
                {
                    m_LastSendTime = NetworkManager.Singleton.NetworkTime;
                    // transform.position = new Vector3(transform.position.x + Input.GetAxis("Horizontal") * Time.fixedDeltaTime * m_Speed, transform.position.y, transform.position.z + Input.GetAxis("Vertical") * Time.fixedDeltaTime * m_Speed);
                    ReceiveServerFrame(new StatePoint(new Vector2(Input.GetAxis("Horizontal"), Input.GetAxis("Vertical")), NetworkManager.Singleton.NetworkTime, transform.position, transform.eulerAngles, m_Player.m_Rigidbody.velocity));
                }
            }
        ...
        private void UpdateClientFrame(float deltaTime)
        {
            ...
            HistoryStatePoint frame = new HistoryStatePoint(deltaTime, deltaTime * (newState.pos - m_ClientPredictedState.pos), newState.rot - m_ClientPredictedState.rot, newState.velocity, m_ClientPredictedState.input);
