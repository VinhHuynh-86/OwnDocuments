...
namespace VH_SYNC
{
    ...
    public class SyncTransform : NetworkBehaviour
    {
        ...
        private Vector2[] m_ServerHistoryDirectionReceiveInput = new Vector2[20];
        ...
        // private List<float> m_SentTime = new List<float>();
        ...
        private void SubmitTransformFromServerToClientRpc(Vector3 position, Vector3 eulerAngles, Vector3 velocity, Vector2 input, float timestamp, ClientRpcParams rpcParams = default)
        {
            ...
            if (IsOwner && IsClient)
            {
                // float sentTime = m_SentTime[0];
                // m_SentTime.RemoveAt(0);
                // Debug.Log("m_Latency=" + (NetworkManager.Singleton.NetworkTime - sentTime));
        ...
        void FixedUpdate()
        {
            ...
            if (IsOwner && IsClient)
            {
                if (NetworkManager.Singleton.NetworkTime - m_LastSendTime >= (1f / FixedSendsPerSecond))
                {
                    ...
                    {
                        m_LastSendTime = NetworkManager.Singleton.NetworkTime;
                        // m_SentTime.Add(NetworkManager.Singleton.NetworkTime);
        ...
        public void ReceiveServerFrame(StatePoint serverFrame)
        {
            ...
            // {
            //     IsFirst = false;
            //     serverFrame.pos = Vector3.zero;
            // }
            // else if(serverFrame.pos == Vector3.zero)
            // {
            //     IsFirst = true;
            // }
