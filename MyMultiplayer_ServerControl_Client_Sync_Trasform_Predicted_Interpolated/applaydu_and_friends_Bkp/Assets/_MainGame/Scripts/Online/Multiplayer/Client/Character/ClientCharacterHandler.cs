...
public class ClientCharacterHandler : NetworkBehaviour
{
    ...
    {
        if (mPlayerController.mState != PLAYER_STATE.FALL)
        {
            if (nextStatus == PLAYER_STATE.FALL)
            {
                mPlayerController.mState = PLAYER_STATE.FALL;
                PlayAnim("",jumpingTrigger);
            }
            else
            {
                if (mPlayerController.mState != nextStatus)
                {
                    mPlayerController.mState = nextStatus;
                    PlayAnim("",mPlayerController.mState == PLAYER_STATE.IDLE ? idleTrigger : walkingTrigger);
                }
            }
        }
        else
