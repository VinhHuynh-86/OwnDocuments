...
{ 
	...
	SubShader
	{
        ...
        Pass
        {
			...
			inline float4 vUnityObjectToClipPos( in float3 pos )
			{
            ...
            Varyings LitPassVertex(Attributes input)
            {
				...
				output.positionCS = vUnityObjectToClipPos(input.positionOS);


				float dist = UNITY_Z_0_FAR_FROM_CLIPSPACE(output.positionCS.z);

				output.positionCS.y -= _CurveStrengthY * dist * dist * _ProjectionParams.x;
				output.positionCS.x -= _CurveStrengthX * dist * dist * _ProjectionParams.y;
            ...
            half4 LitPassFragment(Varyings input) : SV_Target
            {
                ...
                half3 color = input.color * GlobalIllumination(brdfData, bakedGI, surfaceData.occlusion, normalWS, viewDirectionWS);
                ...
                // with a custom one.
				half4 colorTex = tex2D(_MainTex, input.uv) * half4(color, fogFactor);
				return colorTex;
