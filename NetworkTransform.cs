...
namespace MLAPI.Prototyping
{
    ...
    public class NetworkTransform : NetworkBehaviour
    {
        ...
        // Apply transform Client to Server
        // Call in Client side
        [ClientRpc]
        ...
        //[VINH] Apply transform Server
        //Call in Server side
        [ServerRpc]
        private void ApplyTransformToClientFromServerRpc(Vector3 position, Vector3 eulerAngles, ServerRpcParams rpcParams = default)
        {
            if (enabled)
            {
                ApplyTransformInternal(position, Quaternion.Euler(eulerAngles));
        ...
        // Send transform from Client to Server
        [ServerRpc]
        ...
        // Call in Client side
        [ClientRpc]
        private void SubmitTransformFromServerToClientRpc(Vector3 position, Vector3 eulerAngles, ClientRpcParams rpcParams = default)
        {
            if (!enabled)
                return;
            ...
            {
                {
                    ...
                    if ((NetworkManager.Singleton.NetworkTime - info.LastSent >= (1f / FixedSendsPerSecond)) || NetworkManager.Singleton.NetworkTime - info.LastSent >= GetTimeForLerp(receiverPosition.Value, senderPosition.Value))
                    {
                        m_ServerSendInfo.LastSent = NetworkManager.Singleton.NetworkTime;
                        m_ServerSendInfo.LastMissedPosition = null;
                        m_ServerSendInfo.LastMissedRotation = null;
                    ...
                    {
                        m_ServerSendInfo.LastMissedPosition = position;
                        m_ServerSendInfo.LastMissedRotation = Quaternion.Euler(eulerAngles);
                    }
                }
            }
            else
        ...
        //[VINH] Check on client side
        private void CheckForMissedSendsFromServer()
        {
            for (int i = 0; i < NetworkManager.Singleton.ConnectedClientsList.Count; i++)
            {
                if (!m_ServerSendInfo.ContainsKey(NetworkManager.Singleton.ConnectedClientsList[i].ClientId))
                {
                    m_ServerSendInfo.Add(NetworkManager.Singleton.ConnectedClientsList[i].ClientId, new ServerSendInfo()
                    {
                        LastMissedPosition = null,
                        LastMissedRotation = null,
                        LastSent = 0
                ...
                ServerSendInfo info = m_ServerSendInfo[NetworkManager.Singleton.ConnectedClientsList[i].ClientId];
                Vector3? receiverPosition = NetworkManager.Singleton.ConnectedClientsList[i].PlayerObject == null ? null : new Vector3?(NetworkManager.Singleton.ConnectedClientsList[i].PlayerObject.transform.position);
                Vector3? senderPosition = NetworkManager.Singleton.ConnectedClients[OwnerClientId].PlayerObject == null ? null : new Vector3?(NetworkManager.Singleton.ConnectedClients[OwnerClientId].PlayerObject.transform.position);
                ...
                if ((receiverPosition == null || senderPosition == null && NetworkManager.Singleton.NetworkTime - info.LastSent >= (1f / FixedSendsPerSecond)) || NetworkManager.Singleton.NetworkTime - info.LastSent >= GetTimeForLerp(receiverPosition.Value, senderPosition.Value))
                {
                    ...
                    if (info.LastMissedPosition != null && info.LastMissedRotation != null)
                    {
                        ...
                        ApplyTransformToClientFromServerRpc(info.LastMissedPosition.Value, info.LastMissedRotation.Value.eulerAngles);
